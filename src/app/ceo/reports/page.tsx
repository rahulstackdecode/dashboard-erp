...
const initialReports: FileItem[] = [
  {
    id: 1,
    title: "Team Report June 2025",
    name: "Team_Report_June_v1.pdf",
    size: "2.5MB",
    type: "PDF",
    time: "2:00 PM",
  },
  {
    id: 2,
    title: "Team Report June 2024",
    name: "Team_Report_June_v1.doc",
    size: "1.5MB",
    type: "DOC",
    time: "1:45 PM",
  },
...
export default function ReportsPage() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newFiles: FileItem[] = Array.from(files).map((file, index) => {
      ...
      return {
        id: Date.now() + index,
        title: file.name.split(".")[0] || "Untitled File",
        name: file.name,
  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h2 className="font-medium text-[26px] sm:text-[32px] text-[color:var(--heading-color)] leading-snug">
          Reports
        </h2>
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <button
          ...
          className={`text-[18px] font-medium ${
            activeTab === "all" ? "text-[#000]" : "text-gray-500 cursor-pointer"
          }`}
        >
        ...
        <button
          ...
          className={`text-[18px] font-medium ${
            activeTab === "bin" ? "text-[#000]" : "text-gray-500 cursor-pointer"
          }`}
        >
      ...
      <div className="space-y-4">
        {filteredReports.map((file) => (
          <div
            key={file.id}
            className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 rounded-sm transition border border-[#567D8E33] bg-white hover:shadow-[0_3px_5px_rgba(0,0,0,0.04)]"
          >
            {/* Left side */}
            <div className="flex items-start sm:items-center gap-4 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[#626262] text-sm font-light truncate">
                  {file.name} | {file.size}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between sm:justify-end gap-4 mt-3 sm:mt-0 relative group">
              ...
              <div className="relative">
                <button className="p-1 text-gray-500 cursor-pointer hover:text-gray-700">
                ...
                <div className="absolute right-0 top-8 bg-white shadow-md rounded w-28 z-10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition">
                  {activeTab === "all" ? (
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-2 cursor-pointer w-full text-left sm:text-center text-sm rounded-sm transition border border-[#567D8E33] bg-white hover:shadow-[0_3px_5px_rgba(0,0,0,0.04)]"
                    >
        ...
        {filteredReports.length === 0 && (
          <p className="text-center py-6 text-gray-400 text-sm">No files found.</p>
        )}
      </div>
    </div>
  );
